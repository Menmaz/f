...
class MovieCrudController extends CrudController
{
    ...
    protected function setupListOperation()
    {
        ...
        $this->crud->addFilter([
            ...
            // ];
            return Taxonomy::all()->where('type', 'status')->pluck('title', 'slug')->toArray();
        }, function ($value) {
            // $this->crud->addClause('where', 'status', $val);
            $this->crud->addClause('whereHas', 'getStatuses', function($query) use ($value) {
        ...
        $this->crud->addFilter([
            ...
            return Taxonomy::all()->where('type', 'genre')->pluck('title', 'slug')->toArray();
        }, function ($value) { // if the filter is active
            // $this->crud->query = $this->crud->query->whereHas('categories', function ($query) use ($value) {
            //     $query->where('id', $value);
            // });
            $this->crud->addClause('whereHas', 'getCategorys', function($query) use ($value) {
                $query->where('slug', $value);
            });
        });
    ...
    $this->crud->addFilter(
        ...
        // CRUD::addColumn(['name' => 'genres', 'label' => 'Thể loại', 'type' => 'text',]);
        ...
        CRUD::addColumn([
            ...
            'label' => 'Thể loại',
            'type' => 'custom_html',
            'visible_key' => 'genres', // Khai báo khóa dữ liệu sẽ được sử dụng
            'value' => function ($entry) {
                $categories = $entry->getCategorys()->get();
                $html = '<div class="genres">';
                foreach ($categories as $category) {
                    $html .= '<span class="text-danger">' . $category->title . ',</span><br>';
                }
                $html .= '</div>';
                return $html;
            }
    ...
    protected function setupCreateOperation()
    {
        ...
        CRUD::addField(['name' => 'genres', 'label' => 'Thể loại', 'type' => 'checklist', 'tab' => 'Phân loại']);
        // CRUD::addField(['name' => 'genres', 'label' => 'Thể loại', 'type' => 'checklist', 'tab' => 'Phân loại']);
        CRUD::addField(['name' => 'is_sensitive_content', 'label' => 'Đề cử', 'type' => 'boolean', 'tab' => 'Khác']);
